...
To be used if there is a need to differentiate between these interactions. For example, a single click might
select an element, a double click might open it, but double click must not select the element.

This helper class tracks mouse events to differentiate between various click patterns for each mouse button.
When a user interacts with an element, the class:
...
The class maintains separate state for each mouse button (left, right, middle, etc.).
*/
struct UiMouseState
{
